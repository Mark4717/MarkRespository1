...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // User Profile - Default empty/guest state
    ...
    function updateProfileDisplay() {
        ...
        if (userNameElement) userNameElement.textContent = userProfile.name || 'GUEST';
        ...
        if (profileName) profileName.textContent = userProfile.name || 'GUEST USER';
        if (profileType) profileType.textContent = userProfile.type || 'STUDENT';
        if (profileId) profileId.textContent = userProfile.idNumber || 'Not set';
        if (profileDept) profileDept.textContent = userProfile.department || 'Not set';
        if (profileEmail) profileEmail.textContent = userProfile.email || 'Not set';
        if (profileContact) profileContact.textContent = userProfile.contact || 'Not set';
    }
    ...
    function updateMedicalRecordsStats() {
        ...
        const totalCertificates = appointments.filter(a => a.service === 'Medical Certificate' && a.status === 'Completed').length;
        
        let lastVisit = 'N/A';
        if (completedAppointments.length > 0) {
            const sortedVisits = [...completedAppointments].sort((a, b) => new Date(b.date) - new Date(a.date));
            lastVisit = formatShortDate(sortedVisits[0].date);
        }
    ...
    function bookAppointment(formData) {
        const newAppointment = {
            ...
            patientName: userProfile.name || 'Guest User',
            patientId: userProfile.idNumber || 'N/A'
        };
            ...
            if (item.type === 'emergency') {
                ...
                itemElement.innerHTML = `
                    <div class="col-md-8">
                        ...
                        <small><i class="bi bi-calendar3 me-1"></i>${formatDate(item.date)} ${item.time ? `at ${item.time}` : ''}</small>
                    </div>
    ...
    function renderEmergencyRequests() {
        ...
        emergencyRequests.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).forEach(emergency => {
            ...
            emergencyCard.innerHTML = `
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h6 class="fw-bold text-danger">🚨 ${emergency.type}</h6>
    ...
    function renderMedicalRecords() {
        ...
        const sortedRecords = [...completedAppointments].sort((a, b) => new Date(b.date) - new Date(a.date));
        
        medicalRecordsList.innerHTML = sortedRecords.map(record => `
            <div class="card-item-light mb-3 p-3">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <h6 class="fw-bold">${record.service}</h6>
                        <small class="text-muted">${formatDate(record.date)} ${record.time ? `at ${record.time}` : ''}</small>
                        <p class="mt-2 mb-1"><strong>Reason:</strong> ${record.reason || 'Not specified'}</p>
    ...
    function openProfileEditModal() {
        ...
        setTimeout(() => {
            ...
            document.getElementById('saveProfileBtn').addEventListener('click', () => {
                ...
                userProfile = {
                    ...userProfile,
                    name: document.getElementById('editName').value.toUpperCase().trim() || 'GUEST USER',
